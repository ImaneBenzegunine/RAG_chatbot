...
class TuberculosisChatbot:
    def __init__(self):
        ...
        self.text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=512,  # Better for FLAN-T5's limits
            chunk_overlap=64,
            separators=["\n\n", "\n", ".", "?", "!", " ", ""]
        )
    ...
    def generate_response(self, query: str, context: str) -> str:
        ...
        Relevant Context:
        {context[:2000]}  # Strictly limit context length

        Instructions:
        1. Answer concisely but completely
        2. Use ONLY information from the provided context
        3. Include key facts and numbers when available
        4. Structure response in clear paragraphs
    ...
    def _fallback_response(self, chunks: list) -> str:
            ...
            return (
                "I found relevant information in these sources:\n" +
                "\n".join(f"- {source}" for source in sources) +
                "\n\nPlease rephrase your question for a more detailed answer."
            )
    ...
    def ask_question(self, question: str):
        ...
        relevant_chunks = [c for c in self.retrieve_relevant_chunks(question) 
                          if c['score'] > 1.2]  # Filter low-quality matches
